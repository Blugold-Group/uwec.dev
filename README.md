# uwec.dev

This is the code for [uwec.dev](https://uwec.dev), the unofficial student-run guide to the computer science department of [University of Wisconsin - Eau Claire](https://uwec.edu)

If you're a student and you want to contribute code, go ahead! Submit a PR and it'll probably be approved without an issue  

> This website is run solely by students without endorsement or input from any CS faculty.

Inspirations:
- [https://wiki.cs.jmu.edu/](https://wiki.cs.jmu.edu/)
- [https://wiki.cse.lsu.edu/](https://wiki.cse.lsu.edu/)
- [https://uwindsorcss.github.io/wiki/](https://uwindsorcss.github.io/wiki/)

## Todo:
- [ ] Add a clubs section
    - [ ] Page for SACM
    - [ ] Page for cyber club
    - [ ] Page for Rocketry club
    - [ ] Page for electronics club
    - Probably want a set of things to say about each club
        - Meeting time
        - Summary
        - Some pictures
- [ ] FAQ page
    - [ ] What computer do I need?
    - [ ] What's the difference between SE and CS?
    - [ ] How do I get an internship? Should I get an internship? When?
    - [ ] Why join clubs?
    - [ ] How should I use AI in class?
    - [ ] How am I going to get a job if AI exists?
    - [ ] How do I choose what field to go into?
    - [ ] What is research? Should I do it?
- [ ] Page for different fields of CS and how to get into them at UWEC
- [ ] Page detailing and giving tips for all CS classes
    - [ ] Also resources for students to prepare before the class
- [ ] Anecdotes page (like [this](https://acsu.cornell.edu/resources))
- [ ] General tips for the major
- [ ] General tips for college
- [ ] General resources for learning CS and specific fields
- [ ] What to do / where to go when things get tough
- [ ] Cool things to do in the major / department
- [ ] A point of contact for new students to contact with questions (could be someone's email address or a club discord)
- [ ] Places to study
- [ ] Things that CS people should learn and won't learn in class